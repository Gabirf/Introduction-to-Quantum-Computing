\section{Entanglement and and its applications}
...
\noindent
Input: \begin{align*} \ketp \ket{\beta_{00}} = \ketp \otimes \ket{\beta_{00}} &= (\super) \otimes (\ketzz + \ketoo) \\ &= \frac{1}{\sqrt{2}} (\alpha \ketz(\ketzz + \ketoo) + \beta\keto (\ketzz + \ketoo))\end{align*}

\noindent
after CNOT: \\
\(\frac{1}{\sqrt{2}} (\alpha \ketz(\ketzz + \ketoo) + \beta\keto (\ket{\textcolor{red}{1}0} + \ket{\textcolor{red}{0}1}))\) \\
...
\begin{itemize}
    ...
    \item $\vec{v} = (1, 0, 0)$: observable X, eigenstates $\ket{\pm} = \frac{1}{\sqrt{2}}(\super[\pm])$\\
            ...
            \noindent
            If Alice measures eigenvalue 1, the wavefunction will collapse to $\ket{+-} \leadsto$ Bob's qubit is in state $\ket{-}$.\\
...
\begin{itemize}
    ...
    \item Alice: decides randomly whether to measure Q or R.
...
Denote by p(q,r,s,t) the proability that the system is in the state \{Q = q, R = r, S = s, T = t\} before this measurements , then we compute the average value:
\begin{align*}
    \expec[Q \cdot S + R \cdot S + R \cdot T - Q \cdot T] &= \sumX_{p, r, s, t \in {\pm 1}} p(q, r, s, t) \cdot \underbrace{(q \cdot s + r \cdot s + r \cdot t - q \cdot t)}_{\pm 2} \\ &\le \sumX_{p, r, s, t \in {\pm 1}} p(q, r, s, t) \cdot 2 = 2
\end{align*}
