...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.45\textwidth}
        \begin{quantikz}
            ...
            \lstick{$\textcolor{blue}{\bra{e_k}}$}  &  \qw                              & \push{\textcolor{blue}{\ket{e_0}} \textcolor{forest}{\bra{e_0}}} & & \rstick{\textcolor{forest}{$\ket{e_k}$}}
        \end{quantikz}
    \end{subfigure}
\end{figure}
...
\noindent
\textcolor{blue}{
    Theroem: The map $\mathcal{E}$ satisfies A1, A2, A3 if and only if 
    \[\quop = \sumX_k E_k \rho E_k^\dagger \]
    for some some set of complex matrices ${E_k}$ with \(\sumX_k E_k^\dagger E_k \le I\)
}

\noindent
Sketch of proof: \\
$"\Leftarrow"$ to verify A3: Let A be a p.s.d. matrix on enlarged system RQ, then, for any vector \ketp on RQ:

\begin{align*}
    \BraKet{(I \otimes \mathcal{E})(A)} &= \sumX_k \BraKet{(I \otimes E_k) A (I \otimes E_k^\dagger)} \\
                                        &= \sumX_k \BraKet[\psi_k]{A} \ge 0
\end{align*}

\noindent
$"\Rightarrow"$ Principal syste (which $\mathcal{E}$ acts on) denoted Q, dimension n. Introduce another quantum system, labelled R, with some dimension as Q.
