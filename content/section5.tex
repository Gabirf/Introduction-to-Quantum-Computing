...
\textcolor{gray}{\(= (V^T V^*)_{kl} = (V^+ V)_{kl}^* \Leftrightarrow V^+ V = I \)} (V is unitary)
...
= \sumX_{k, j} v_{ik} \; \omega_{jk}^* \ket{\widetilde{\varphi}_j} = \sumX_j (V \cdot \omega^+)_{jk} \ket{\widetilde{\varphi}_j}\) and \(V \cdot \omega^+\) is (as a product of unitary matrice) again unitary. \\
